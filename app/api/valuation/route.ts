...
async function addNote(contactId: string, address: string, cityZip: string, timeline: string) {
  await fetch(`${GHL_BASE}/contacts/${contactId}/notes`, {
    method: "POST",
    headers: ghlHeaders(),
    body: JSON.stringify({
      body: `Home valuation request from todd-spencer.com/new-braunfels-home-valuation\n\nProperty address: ${address}\nCity/ZIP: ${cityZip}\nSelling timeline: ${timeline || "Not specified"}`,
    }),
...
async function addTag(contactId: string) {
  await fetch(`${GHL_BASE}/contacts/${contactId}/tags`, {
    method: "POST",
    headers: ghlHeaders(),
    body: JSON.stringify({ tags: ["heypearl", "home-valuation-lead"] }),
...
export async function POST(request: NextRequest) {
  ...
  const { firstName, lastName, email, phone, address, cityZip, timeline } = body as {
    firstName?: string;
    lastName?: string;
    email?: string;
    phone?: string;
  ...
  if (!firstName || !lastName || !email || !phone || !address || !cityZip) {
  ...
  const createRes = await fetch(`${GHL_BASE}/contacts/`, {
    ...
    body: JSON.stringify({
      ...
      source: "todd-spencer.com/new-braunfels-home-valuation",
    }),
  });
  ...
  if (!createRes.ok) {
    if (createRes.status === 400 && createData.meta?.contactId) {
      ...
      await Promise.all([
        addNote(contactId, address, cityZip, timeline ?? ""),
        addTag(contactId),
      ]);
